...
// “[texOffset] is set automatically by Processing and contains the vector (1/width, 1/height), 
//  with width and height being the resolution of the texture. These values are precisely 
//  the offsets along the horizontal and vertical directions needed to sample the color from the 
//  texels around vertTexCoord.st. For example, vertTexCoord.st + vec2(texOffset.s, 0) is the texel 
...
void main(void)
{
	...
	float srcTexColor	= texture2D( srcTexture, vec2( texCoord.s, 1.0 - texCoord.t) ).r; // flip vertically to compensate for OpenGL's coordinate system
...
//============================================================================
	...
	u += du*0.6; // semi-arbitrary scaling (reduces blow-ups?)
	v += dv*0.6;


	float red    = clamp( u, 0.0, 1.0 ); // Constrain the value to [0.0,1.0]
	float green  = 1.0 - u/v;            //  ???
	float blue   = clamp( v, 0.0, 1.0 ); // Constrain the value to [0.0,1.0]

	gl_FragColor = vec4( red, green, blue, 1.0 ); // output new (U,V)
}
